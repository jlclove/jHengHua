...
@Repository
@Lazy(true)
public class HenghuaSampleDetailDao extends LocalRWDao<HenghuaSampleDetail> {
	 ...
	 */
	public int batchReplaceInto(List<HenghuaSampleDetail> sampleDetailList, boolean deleteOutOfTime) {
		Date updateTime = new Date();
		int effectCount = super.batchOperate(100, sampleDetailList, new BatchOperation(super.getSqlSession()) {
			@Override
			public int process(List<?> dataList) {
				...
				for (Object detail : dataList) {
					itemList.addAll(createDetailItem((HenghuaSampleDetail) detail));
				}
				if (itemList.size() > 0) {
	...
	public int deleteByDetailId(String detailId) {
		return super.getSqlSession().delete("HenghuaSampleDetail.delete", detailId);
	}

	public List<Map<String, String>> createDetailItem(HenghuaSampleDetail detail) {
	 ...
	 */
	public List<HenghuaSampleDetail> findSampleDetails(Map<String, Object> map) {
	 ...
	 */
	public HenghuaSampleDetail findSampleDetail(String detailId) {
