...
public class HenghuaService {
	 ...
	 * @author jail
	 */
	public List<HenghuaSample> search(long id, String level, String style, String gramWeight, String season,
			String zuzhi, String fabrics, String colorTypes, String clearTypes, String materialTypes) {
		List<HenghuaSample> list = this.henghuaSampleDao.search(id, level, style, gramWeight, season, zuzhi, fabrics,
				colorTypes, clearTypes, materialTypes);
		bindSampleDetailData(list);
		return list;
	}
	...
	public Map<String, List<HenghuaSampleDetail>> getSampleDetailList(Map<String, Object> map) {
		return this.henghuaSampleDetailDao.findSampleDetails(map).stream()
				.collect(Collectors.groupingBy(HenghuaSampleDetail::getCardIds));
	}
	 ...
	 */
	/*
	 * @Cacheable(value = "clothDetail", key = "#serialNo") public
	 * HenghuaClothDetail getClothDetail(String serialNo) { String r =
	 * restHenghua.restClothDetail(serialNo); r = CoderUtil.decodeUnicode(r);
	 * JSONObject obj = JSONObject.parseObject(r); r =
	 * obj.getString("result").replaceAll("\\[\\[", "").replaceAll("\\]\\]",
	 * ""); HenghuaClothDetail detail = JSONObject.parseObject(r,
	 * HenghuaClothDetail.class); return detail; }
	 */
		...
		if (this.clothTypeCacheMap == null || this.clothTypeCacheMap.isOutOfTime()) {
			...
			try {
				...
				ar.forEach(a -> {
					JSONObject o = (JSONObject) a;
					o.keySet().forEach(
							key -> this.clothTypeCacheMap.put(key, Arrays.asList(o.getString(key).split(","))));
				});
			} catch (Exception e) {
