...
<fmt:bundle basename="site">
	<div class="store-page container-fluid wrapper">
		<div class="store-info" style="background-image: url('/imgs/${store.picPath}');">
			<div class="overlay">
				<h3>${store.storeName}</h3>
				<a href="/stores?countryId=${country.id}" class="arrow-left"><fmt:message
						key="store_back" /></a>
				<h4 class="mt30">
					<fmt:message key="store_address" />
				</h4>
				<p>${store.province}${store.address}</p>
				<p>${store.zipCode}${country.name}</p>
				<p>
					<fmt:message key="store_phone" />
					: ${store.phone}
				</p>
			</div>
		</div>
		<div class="row mt40 pl10 pr10">
			<div class="col-md-4">
				<h4>
					<fmt:message key="store_opentime" />
				</h4>
				<hr>
				<div class="clearfix">
					<span class="pull-left"><fmt:message key="store_monday" /></span> <span
						class="pull-right">${store.workTime1}</span>
				</div>
				<div class="clearfix">
				...
				<div class="clearfix">
					<span class="pull-left"><fmt:message key="store_friday" /></span> <span
						class="pull-right">${store.workTime5}</span>
				</div>
				<div class="clearfix">
				...
				<div class="clearfix">
					<span class="pull-left"><fmt:message key="store_sunday" /></span> <span
						class="pull-right">${store.workTime7}</span>
				</div>
			</div>
		</div>
	</div>
...
    window.onload = function() {
		var img = new Image();
		img.src = '/imgs/${store.picPath}';
		if (img.width == 0) {
			$('.store-info').css('background-image','url(/static/images/tmp-store-big.jpg)');
		}
	}
</script>